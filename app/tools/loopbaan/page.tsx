...
type Step = "input" | "generating" | "cv-ready" | "vacancies" | "tailoring" | "tailored" | "error";
type Lang = "nl" | "en";
type Template = "modern" | "classic" | "bold" | "minimal" | "sidebar" | "executive";
type ScopeFilter = "all" | "nl" | "remote" | "international";
...
// ─── Template switcher ─────────────────────────────────────────────────────────
function TemplateSwitcher({ template, setTemplate, photo, photoName, photoDataUrl, onPhotoClick, onPhotoClear }: {
  template: Template;
      ...
      <div className="flex items-center gap-3 pt-1 border-t border-gray-100">
        {photoDataUrl ? (
          <div className="flex items-center gap-2 flex-1">
            ...
            <button onClick={onPhotoClick} className="text-xs text-blue-600 hover:underline">Wijzigen</button>
            <button onClick={onPhotoClear} className="text-xs text-red-400 hover:underline">Verwijderen</button>
          </div>
        ) : (
          <button onClick={onPhotoClick} className="flex items-center gap-2 text-xs text-gray-500 hover:text-blue-600 transition-colors">
...
// ─── Main page ────────────────────────────────────────────────────────────────
export default function LoopbaanPage() {
  ...
  async function generateAndSearch() {
    ...
    setStep("generating");
    try {
      const res = await fetch("/api/tools/cv-loopbaan", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ profileText, lang }),
      });
      const data = await res.json();
      if (data.error) throw new Error(data.error);
      setCv(data.cv);
  ...
  async function tailorCV(vacancy: Vacancy) {
    setSelectedVacancy(vacancy);
    setStep("tailoring");
    try {
      const res = await fetch("/api/tools/cv-tailor", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ profileText, jobDescription: vacancy.description, lang }),
      });
      const data = await res.json();
      if (data.error) throw new Error(data.error);
      setTailoredCv(data);
      ...
      setErrorMsg(err instanceof Error ? err.message : "CV aanpassen mislukt");
      setStep("error");
    }
  }

  async function exportCvToDocx(cvData: CV) {
    const {
    ...
    if (template === "sidebar") {
      const fill = "1E3A5F";
      const contactParts = [cvData.contact.location, cvData.contact.email, cvData.contact.phone, cvData.contact.linkedin].filter(Boolean) as string[];
      ...
      left.push(new Paragraph({ children: [new TextRun({ text: cvData.name, bold: true, size: 26, color: "FFFFFF" })], shading: { fill }, spacing: { before: photoData ? 0 : 120, after: 40 }, indent: { left: 120 } }));
      left.push(new Paragraph({ children: [new TextRun({ text: cvData.title, size: 20, color: "93C5FD" })], shading: { fill }, spacing: { after: 80 }, indent: { left: 120 } }));
      ...
      if (cvData.skills.length > 0) {
        ...
        for (const sk of cvData.skills) left.push(new Paragraph({ children: [new TextRun({ text: `• ${sk}`, size: 18, color: "E2E8F0" })], shading: { fill }, spacing: { after: 40 }, indent: { left: 120 } }));
      }
      if (cvData.languages.length > 0) {
        left.push(leftSidebarLabel(nl ? "Talen" : "Languages"));
        for (const l of cvData.languages) left.push(new Paragraph({ children: [new TextRun({ text: l, size: 18, color: "E2E8F0" })], shading: { fill }, spacing: { after: 40 }, indent: { left: 120 } }));
      }
      if (cvData.certifications && cvData.certifications.length > 0) {
        left.push(leftSidebarLabel(nl ? "Certificaten" : "Certifications"));
        for (const c of cvData.certifications) left.push(new Paragraph({ children: [new TextRun({ text: `• ${c}`, size: 18, color: "E2E8F0" })], shading: { fill }, spacing: { after: 40 }, indent: { left: 120 } }));
      }
      ...
      right.push(new Paragraph({ text: cvData.summary, spacing: { after: 120 } }));
      ...
      if (cvData.education.length > 0) {
        ...
        for (const edu of cvData.education) right.push(new Paragraph({ children: [new TextRun({ text: edu.degree, bold: true, size: 22 }), new TextRun({ text: `  ${edu.institution}`, size: 22, color: "6B7280" }), new TextRun({ text: `  ${edu.period}`, size: 18, color: "9CA3AF" })], spacing: { before: 80, after: 60 } }));
      }
      ...
      if (template === "modern") {
        const lc = [
          new Paragraph({ children: [new TextRun({ text: cvData.name, bold: true, size: 52, color: "FFFFFF" })], shading: { fill: "2563EB" }, indent: { left: 200 }, spacing: { before: 120, after: 0 } }),
          new Paragraph({ children: [new TextRun({ text: cvData.title, size: 24, color: "BFDBFE" })], shading: { fill: "2563EB" }, indent: { left: 200 }, spacing: { after: 0 } }),
        ...
        else { children.push(new Paragraph({ children: [new TextRun({ text: cvData.name, bold: true, size: 52, color: "FFFFFF" })], shading: { fill: "2563EB" }, spacing: { before: 0, after: 0 }, indent: { left: 200, right: 200 } })); children.push(new Paragraph({ children: [new TextRun({ text: cvData.title, size: 24, color: "BFDBFE" })], shading: { fill: "2563EB" }, spacing: { after: 0 }, indent: { left: 200, right: 200 } })); children.push(new Paragraph({ children: [new TextRun({ text: contactParts.join("  |  "), size: 18, color: "93C5FD" })], shading: { fill: "1D4ED8" }, spacing: { after: 200 }, indent: { left: 200, right: 200 } })); }
      } else if (template === "classic") {
        const lc = [
          new Paragraph({ children: [new TextRun({ text: cvData.name, bold: true, size: 44, color: "111827" })], spacing: { after: 60 } }),
          new Paragraph({ children: [new TextRun({ text: cvData.title, size: 24, color: "6B7280", italics: true })], spacing: { after: 80 } }),
        ...
        else { children.push(new Paragraph({ children: [new TextRun({ text: cvData.name, bold: true, size: 44, color: "111827" })], alignment: AlignmentType.CENTER, spacing: { after: 60 } })); children.push(new Paragraph({ children: [new TextRun({ text: cvData.title, size: 24, color: "6B7280", italics: true })], alignment: AlignmentType.CENTER, spacing: { after: 80 } })); children.push(new Paragraph({ children: [new TextRun({ text: contactParts.join("  ·  "), size: 18, color: "9CA3AF" })], alignment: AlignmentType.CENTER, border: { bottom: { style: BorderStyle.SINGLE, size: 8, color: "E5E7EB", space: 8 } }, spacing: { after: 200 } })); }
      } else if (template === "bold") {
        const lc = [
          new Paragraph({ children: [new TextRun({ text: cvData.name, bold: true, size: 56, color: "FFFFFF" })], shading: { fill: "111827" }, indent: { left: 200 }, spacing: { before: 120, after: 0 } }),
          new Paragraph({ children: [new TextRun({ text: cvData.title, size: 24, color: "2DD4BF" })], shading: { fill: "111827" }, indent: { left: 200 }, spacing: { after: 0 } }),
        ...
        else { children.push(new Paragraph({ children: [new TextRun({ text: cvData.name, bold: true, size: 56, color: "FFFFFF" })], shading: { fill: "111827" }, spacing: { before: 0, after: 0 }, indent: { left: 200, right: 200 } })); children.push(new Paragraph({ children: [new TextRun({ text: cvData.title, size: 24, color: "2DD4BF" })], shading: { fill: "111827" }, spacing: { after: 0 }, indent: { left: 200, right: 200 } })); children.push(new Paragraph({ children: [new TextRun({ text: contactParts.join("   "), size: 18, color: "6EE7B7" })], shading: { fill: "1F2937" }, spacing: { after: 200 }, indent: { left: 200, right: 200 } })); }
      } else if (template === "executive") {
        const lc = [
          new Paragraph({ children: [new TextRun({ text: cvData.name, bold: true, size: 60, color: "FFFFFF" })], shading: { fill: "0F172A" }, indent: { left: 200 }, spacing: { before: 160, after: 0 } }),
          new Paragraph({ children: [new TextRun({ text: cvData.title, size: 24, color: "F59E0B" })], shading: { fill: "0F172A" }, indent: { left: 200 }, spacing: { after: 0 } }),
        ...
        else { children.push(new Paragraph({ children: [new TextRun({ text: cvData.name, bold: true, size: 60, color: "FFFFFF" })], shading: { fill: "0F172A" }, spacing: { before: 0, after: 0 }, indent: { left: 200, right: 200 } })); children.push(new Paragraph({ children: [new TextRun({ text: cvData.title, size: 24, color: "F59E0B" })], shading: { fill: "0F172A" }, spacing: { after: 0 }, indent: { left: 200, right: 200 } })); children.push(new Paragraph({ children: [new TextRun({ text: contactParts.join("   "), size: 18, color: "94A3B8" })], shading: { fill: "1E293B" }, spacing: { after: 200 }, indent: { left: 200, right: 200 } })); }
      } else {
        // minimal
        const cp = contactParts.join("   ");
        if (photoData) {
          const lc = [new Paragraph({ children: [new TextRun({ text: cvData.name, bold: true, size: 48, color: "111827" })], spacing: { after: 40 } }), new Paragraph({ children: [new TextRun({ text: cvData.title, size: 22, color: "6B7280" })], spacing: { after: 60 } }), new Paragraph({ children: [new TextRun({ text: cp, size: 18, color: "9CA3AF" })], border: { bottom: { style: BorderStyle.SINGLE, size: 2, color: "E5E7EB", space: 6 } }, spacing: { after: 200 } })];
          children.push(makeHeaderTable(lc, "FFFFFF", "FFFFFF")); children.push(new Paragraph({ text: "", spacing: { after: 40 } }));
        } else {
          children.push(new Paragraph({ children: [new TextRun({ text: cvData.name, bold: true, size: 48, color: "111827" })], spacing: { after: 40 } })); children.push(new Paragraph({ children: [new TextRun({ text: cvData.title, size: 22, color: "6B7280" })], spacing: { after: 60 } })); children.push(new Paragraph({ children: [new TextRun({ text: cp, size: 18, color: "9CA3AF" })], border: { bottom: { style: BorderStyle.SINGLE, size: 2, color: "E5E7EB", space: 6 } }, spacing: { after: 200 } }));
        }
      }

      children.push(sectionLabel(nl ? "Profiel" : "Profile"));
      children.push(new Paragraph({ text: cvData.summary, spacing: { after: 120 } }));
      ...
      if (cvData.education.length > 0) {
        ...
        for (const edu of cvData.education) children.push(new Paragraph({ children: [new TextRun({ text: edu.degree, bold: true, size: 22 }), new TextRun({ text: `  ${edu.institution}`, size: 22, color: "6B7280" }), new TextRun({ text: `  ${edu.period}`, size: 18, color: "9CA3AF" })], spacing: { before: 80, after: 60 }, ...(template === "executive" ? { indent: { left: 120 } } : {}) }));
      }
      if (cvData.skills.length > 0) {
        children.push(sectionLabel(nl ? "Vaardigheden" : "Skills"));
        children.push(new Paragraph({ text: cvData.skills.join("  ·  "), spacing: { after: 80 }, ...(template === "executive" ? { indent: { left: 120 } } : {}) }));
      }
      if (cvData.languages.length > 0) {
        children.push(sectionLabel(nl ? "Talen" : "Languages"));
        children.push(new Paragraph({ text: cvData.languages.join("  ·  "), spacing: { after: 80 }, ...(template === "executive" ? { indent: { left: 120 } } : {}) }));
      }
      if (cvData.certifications && cvData.certifications.length > 0) {
        children.push(sectionLabel(nl ? "Certificaten" : "Certifications"));
        for (const cert of cvData.certifications) children.push(new Paragraph({ text: cert, bullet: { level: 0 } }));
      }
    }
    ...
    a.href = url;
    a.download = `CV_${cvData.name.replace(/\s+/g, "_")}_${template}.docx`;
    a.click();
  ...
  const filteredVacancies = scopeFilter === "all" ? vacancies : vacancies.filter((v) => v.scope === scopeFilter);

  return (
    <div className="min-h-screen bg-gray-50">
      <ToolNav label="💼 CV & Vacatures" />
      ...
      <main className="max-w-2xl mx-auto px-4 py-10">
        ...
        {step === "input" && (
          <div className="bg-white rounded-2xl shadow-lg p-8 flex flex-col gap-6">
            <div>
              <h1 className="text-xl font-bold text-gray-900 mb-1">CV & Vacatures</h1>
              <p className="text-gray-500 text-sm">Upload je CV en ontdek passende vacatures. Pas je CV automatisch aan op een functie die je aanspreekt. Kost 2 credits.</p>
            </div>
            ...
            <button onClick={generateAndSearch} disabled={!profileText.trim()}
              className="bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 disabled:opacity-40 text-white rounded-xl py-3 text-sm font-semibold transition-all shadow-lg shadow-blue-100">
              Maak CV aan en zoek vacatures →
            </button>
          </div>
        )}
        ...
        {step === "generating" && (
          <div className="bg-white rounded-2xl shadow-lg p-8 flex flex-col items-center gap-4 py-16">
            ...
            <p className="text-sm text-gray-700 font-medium">
              {lang === "nl" ? "CV wordt gemaakt en vacatures worden gezocht..." : "Generating CV and searching vacancies..."}
            </p>
            <p className="text-xs text-gray-400">Dit duurt 15–30 seconden</p>
          </div>
        )}
        ...
        {step === "cv-ready" && cv && (
          <div className="flex flex-col gap-4">
            <TemplateSwitcher
              ...
              onPhotoClear={() => { setPhotoDataUrl(null); setPhotoName(""); }}
            />

            <CVPreview cv={cv} template={template} lang={lang} photo={photoDataUrl} />

            <div className="flex gap-3">
              <button onClick={() => exportCvToDocx(cv)}
                className="flex-1 bg-white border border-gray-200 hover:bg-gray-50 text-gray-700 rounded-xl py-3 text-sm font-medium transition-colors">
                📥 Download als Word
              </button>
              <button onClick={() => setStep("vacancies")}
                className="flex-1 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white rounded-xl py-3 text-sm font-semibold transition-all shadow-lg shadow-blue-100">
                🔍 Bekijk passende vacatures →
              </button>
            </div>

            <div className="text-center">
              <button onClick={reset} className="text-xs text-gray-400 hover:text-gray-600 transition-colors">Opnieuw beginnen</button>
            </div>
          </div>
        )}
        ...
        {step === "vacancies" && (
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-bold text-gray-900">Gevonden vacatures voor jouw profiel</h2>
              <span className="bg-blue-100 text-blue-700 text-xs font-semibold px-2 py-1 rounded-full">{vacancies.length} gevonden</span>
            </div>
            ...
            <div className="flex gap-2 overflow-x-auto pb-1">
              ...
              ] as { value: ScopeFilter; label: string }[]).map((tab) => (
                <button key={tab.value} onClick={() => setScopeFilter(tab.value)}
                  className={`whitespace-nowrap px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${scopeFilter === tab.value ? "border-blue-500 bg-blue-50 text-blue-700" : "border-gray-200 text-gray-600 hover:border-gray-300"}`}>
                  {tab.label}
                </button>
              ))}
            </div>
            ...
            {filteredVacancies.length === 0 ? (
              <div className="bg-white rounded-2xl shadow-lg p-8 text-center">
                <p className="text-gray-500 text-sm">Geen vacatures gevonden. Probeer je CV aan te passen of probeer het later opnieuw.</p>
              </div>
            ) : (
              <div className="flex flex-col gap-3">
                {filteredVacancies.map((v) => (
                  <div key={v.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 flex flex-col gap-3">
                    <div>
                      ...
                      <p className="text-sm text-gray-500 mt-0.5">{v.company} · {v.location}</p>
                      <div className="flex items-center gap-2 mt-1.5">
                        <ContractBadge type={v.contractType} />
                        {v.salary && <span className="text-xs text-gray-400">{v.salary}</span>}
                      </div>
                    </div>
                    <p className="text-sm text-gray-600 leading-relaxed">{v.description.slice(0, 120)}{v.description.length > 120 ? "…" : ""}</p>
                    <div className="flex gap-2 pt-1">
                      <button onClick={() => tailorCV(v)}
                        className="flex-1 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white rounded-xl py-2 text-xs font-semibold transition-all">
                        🎯 Pas CV aan op deze functie
                      </button>
                      <a href={v.url} target="_blank" rel="noopener noreferrer"
                        className="px-3 py-2 border border-gray-200 hover:bg-gray-50 text-gray-600 rounded-xl text-xs font-medium transition-colors">
                        Bekijk vacature →
                      </a>
                    </div>
                  </div>
            ...
            {cv && (
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-xs text-gray-400">{cv.title}</p>
                </div>
                <button onClick={() => setStep("cv-ready")} className="text-xs text-blue-600 hover:underline font-medium">← Terug naar CV</button>
              </div>
            )}
          </div>
        )}
        ...
        {step === "tailoring" && (
          <div className="bg-white rounded-2xl shadow-lg p-8 flex flex-col items-center gap-4 py-16">
            ...
            <p className="text-sm text-gray-700 font-medium text-center">
              CV wordt aangepast op <strong>{selectedVacancy?.title}</strong> bij <strong>{selectedVacancy?.company}</strong>...
            </p>
        ...
        {/* ── TAILORED ── */}
        {step === "tailored" && tailoredCv && selectedVacancy && (
          <div className="flex flex-col gap-4">
            <div className="bg-white rounded-2xl shadow-lg p-5">
              <h2 className="text-lg font-bold text-gray-900">CV gericht op {selectedVacancy.title}</h2>
              <div className="flex items-center gap-2 mt-1">
                <span className="text-sm text-gray-500">bij</span>
                <span className="inline-block bg-blue-100 text-blue-700 text-xs font-semibold px-2 py-0.5 rounded">{selectedVacancy.company}</span>
              </div>
            </div>
            ...
            <CVPreview cv={tailoredCv} template={template} lang={lang} photo={photoDataUrl} />

            <div className="flex gap-3">
              <button onClick={() => exportCvToDocx(tailoredCv)}
                className="flex-1 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white rounded-xl py-3 text-sm font-semibold transition-all shadow-lg shadow-blue-100">
                📥 Download als Word
              </button>
              <button onClick={() => setStep("vacancies")}
                className="flex-1 bg-white border border-gray-200 hover:bg-gray-50 text-gray-700 rounded-xl py-3 text-sm font-medium transition-colors">
                ← Andere vacature bekijken
              </button>
            </div>

            <div className="text-center">
              <button onClick={reset} className="text-xs text-gray-400 hover:text-gray-600 transition-colors">Of begin opnieuw</button>
            </div>
          </div>
        )}
        ...
        {/* Hidden photo input (needed at all steps) */}
        <input ref={photoInputRef} type="file" accept="image/png,image/jpeg,image/webp" className="hidden" onChange={handlePhoto} />
      </main>
    </div>
  );
